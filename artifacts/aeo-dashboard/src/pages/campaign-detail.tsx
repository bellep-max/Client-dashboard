...
import { ArrowLeft, Loader2, Key, Lock, TrendingUp, TrendingDown, Minus } from "lucide-react";
        ...
        <CardContent className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="md:col-span-2 space-y-1">
              <div className="flex items-center gap-1.5">
                <p className="text-xs font-medium uppercase tracking-wide text-muted-foreground">Search Address</p>
                <Lock className="w-3 h-3 text-muted-foreground" aria-label="Locked" />
              </div>
              <p className="text-sm">{plan.searchAddress?.trim() || <span className="text-muted-foreground">—</span>}</p>
              <p className="text-xs text-muted-foreground">Changing the search address resets the initial report. Contact your admin to update.</p>
            </div>
            <div className="space-y-1">
          ...
          <div className="border-t pt-4 space-y-4">
            <div>
              ...
              <p className="text-xs text-muted-foreground">Manual entry — fill in if you have it.</p>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <Field
                label="Start Date"
                value={plan.subscriptionStartDate
                  ? format(new Date(plan.subscriptionStartDate), "MMM d, yyyy")
                  : null}
              />
              <Field
                label="Next Billing Date"
                value={plan.nextBillingDate
              ...
              {keywordsLoading ? (
                ...
                <TableRow>
                  <TableCell colSpan={6} className="text-center py-6 text-muted-foreground text-sm">
                    No keywords are linked to this campaign yet. Keywords are added by your admin.
                  </TableCell>
                </TableRow>
              ) : (
                keywords.map((kw) => {
                  ...
                  return (
                    <TableRow key={kw.id} className="hover:bg-muted/40">
                      <TableCell className="font-medium">
                        <div className="flex items-center gap-1.5">
                          {isLocked && <Lock className="w-3 h-3 text-amber-500" title="Locked — top ranking achieved" />}
                      ...
                      <TableCell className="text-center">
                        {initial != null && current != null ? (
                          <span className={`flex items-center justify-center gap-1 text-xs font-medium ${trendColor}`}>
